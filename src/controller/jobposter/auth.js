    ...
    posterRegister.findOne({ email: req.body.email }).exec(async (error, user) => {
        ...
        _user.save((error, user) => {
            ...
            if (user) {
                ...
                const { _id, contactNumber, email } = user;
                return res.status(200).json({
                    message:'success', 
                    user: { _id, contactNumber, email },   
                }); 
            } 
        });
    });
...
exports.sendotp = async (req, res, next) => {
    posterRegister.findOne({ email: req.body.email, contactNumber: req.body.contactNumber }).exec(async (error, user) => {
        if (user) {
